...
var out: [Int] = []
let c = a.merge(with: b).sink(receiveValue: { out.append($0) })
_ = c

vs.schedule(after: VTime(t: 10), tolerance: .zero, options: nil) { a.send(1) }
vs.schedule(after: VTime(t: 20), tolerance: .zero, options: nil) { b.send(2) }
vs.schedule(after: VTime(t: 30), tolerance: .zero, options: nil) { a.send(3) }
vs.schedule(after: VTime(t: 40), tolerance: .zero, options: nil) { b.send(4) }
vs.schedule(after: VTime(t: 50), tolerance: .zero, options: nil) { a.send(5) }
vs.schedule(after: VTime(t: 60), tolerance: .zero, options: nil) { b.send(6) }
