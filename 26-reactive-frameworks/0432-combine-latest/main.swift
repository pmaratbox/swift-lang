...
var out: [String] = []
let c = a.combineLatest(b).map { "(\($0), \($1))" }.sink(receiveValue: { out.append($0) })
_ = c

vs.schedule(after: VTime(t: 10), tolerance: .zero, options: nil) { a.send(1) }
vs.schedule(after: VTime(t: 20), tolerance: .zero, options: nil) { b.send(10) }
vs.schedule(after: VTime(t: 30), tolerance: .zero, options: nil) { a.send(2) }
vs.schedule(after: VTime(t: 100), tolerance: .zero, options: nil) { a.send(completion: .finished); b.send(completion: .finished) }
vs.run()
